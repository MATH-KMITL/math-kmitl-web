...
var docRef = firebase.database().ref(`course/${course}/document`);
var docStgRef = firebase.storage().ref(`course/${course}`);
            ...
            if (!docList.some(doc => doc.name === docName)) {
                ...
                docStgRef.child(fileName).put(file).then(snapshot => {
                    ...
                    docRef.set(docList).then(() => {
                        swal('upload สำเร็จ', 'เสร็จสิ้น!', "success").then(function (value) { window.location.href = `document-manager-${course}.html` })
                    })
                })
            } else {
