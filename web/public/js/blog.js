...
async function initial() {
    await spRef.once('value', function (snapshot) {
        ...
        Object.keys(snapshot.val()).reverse().forEach(function (key, index) {
            var year = key;
            var data = snapshot.val()[key]
            // add menu  
            if (index === 0) {
                $('#menu').append(`<button class="label-gallery txt26 trans-0-4 is-actived" id="firstGroup" data-filter=".slide${year}">${year}</button>`);
            } else {
            ...
            Object.keys(data).sort((a, b) => (a.split('M_')[1] - b.split('M_')[1])).forEach(function (spKey) {
                $(`#${year}`).append(spHtml(spKey, data[spKey]))
            })
        });
    });
}
...
initial().then(() => {
    ...
    // initial 
    $topeContainer.isotope({ filter: $('#firstGroup').attr('data-filter') });
    ...
    $(labelGallerys).each(function () {
        $(this).on('click', function () {
            ...
            for (var i = 0; i < labelGallerys.length; i++) {
                ...
                $(labelGallerys[i]).removeClass('current');
            }
...
function menuHtml(year) {
    let html = ''
    html += `<button class="label-gallery txt26 trans-0-4" data-filter=".slide${year}">`
    html += year
    html += '</button>'
    return html
}

function groupHtml(year) {
    let html = ''
    html += `<div class="container slide${year}">`
    html += `<div class="row slide">`
    html += '<table class="table wow table-hover">'
    html += '<thead>'
    html += '<tr>'
    html += '<th scope="col">รหัส</th>'
    html += '<th scope="col">ชื่อเรื่องปัญหาพิเศษ</th>'
    html += '</tr>'
    html += '</thead>'
    html += `<tbody id="${year}">`
    html += '</tbody>'
